...
localStorageBtn.addEventListener('click', () => {
  ...
  //make copy of the old array and add item to it
...
sessionStorageBtn.addEventListener('click', () => {
  ...
  // Get previous data  OR an empty array
  const previousData = JSON.parse(sessionStorage.getItem('sessionExample')) || [];
  // Set item to a stringified version of an array with the old and new tasks
  sessionStorage.setItem('sessionExample', JSON.stringify([...previousData, task]));
});
...
(() => {
  ...
  const dataInLocalStorage = JSON.parse(localStorage.getItem('localExample')) || [];
  ...
  const dataInSessionStorage = JSON.parse(sessionStorage.getItem('localExample')) || [];
